...
function App() {
 ...
 // Also detect "TraitName: Resistance to X damage" pattern (e.g. Hellish Resistance)
 const resistParts = entry.notes.split(' | ');
 resistParts.forEach(part=>{
 const rm = part.match(/^[^:]+:\s*[Rr]esistance to ([^|]+?) damage/i);
 if(rm){
 const dmgType = rm[1].trim();
 if(!results.find(r=>r.toLowerCase()===dmgType.toLowerCase())) results.push(dmgType);
                      }
                    });
                  });
 return results;
