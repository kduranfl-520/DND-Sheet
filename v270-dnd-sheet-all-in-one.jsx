...
function App() {
 ...
 onClick={()=>setOpenTrait(openTrait==="race"?null:"race")}>
                          ...
                          {openTrait==="race"&&(()=>{
 let raceCardExtra = null;
 if(isHuman){
 raceCardExtra = (
                            <div style={{padding:"8px 0 4px"}}>
                              <div style={{fontSize:12,fontWeight:600,color:"var(--text)",marginBottom:6}}>
                                Extra Language <span style={{fontSize:10,color:"var(--muted)",fontWeight:400}}>(Human trait)</span>
                              </div>
                              <select
 value={humanExtraLang}
 onChange={e=>{const langs=char.languages||[];const rest=langs.slice(1);setF("languages",[e.target.value,...rest]);}}
 style={{width:"100%",background:"var(--input)",border:"1px solid "+(humanExtraLang?"var(--green)":"var(--border)"),borderRadius:8,padding:"6px 10px",fontSize:12,color:humanExtraLang?"var(--text)":"var(--subtle)",outline:"none",fontFamily:"inherit"}}>
 ...
 raceCardExtra = halfElfChosen.length<2?(
                            <div style={{padding:"8px 0 4px"}}>
                              <div style={{fontSize:12,fontWeight:600,color:"var(--text)",marginBottom:6}}>
                                Ability Score Increase <span style={{fontSize:10,color:"var(--muted)",fontWeight:400}}>(+2 CHA · +1 to two others)</span>
                              </div>
                              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                                {["STR","DEX","CON","INT","WIS"].map(stat=>{
 const chosen=halfElfChosen.includes(stat);
 const full=halfElfChosen.length>=2&&!chosen;
 return(<button key={stat} onClick={()=>{if(full)return;const next=chosen?halfElfChosen.filter(s=>s!==stat):[...halfElfChosen,stat];setF("halfElfBonusStats",next);}} style={{padding:"4px 12px",borderRadius:8,fontSize:12,fontWeight:700,border:"1px solid "+(chosen?"rgba(99,102,241,0.5)":full?"var(--divider)":"var(--border)"),background:chosen?"rgba(99,102,241,0.12)":full?"transparent":"var(--input)",color:chosen?"var(--accent)":full?"var(--subtle)":"var(--text)",cursor:full?"default":"pointer",fontFamily:"inherit",transition:"all .15s"}}>{stat}{chosen&&" +1"}</button>);
                                })}
                              </div>
                              <div style={{fontSize:10,color:"var(--subtle)",marginTop:4,fontStyle:"italic"}}>{2-halfElfChosen.length} more to choose</div>
                            </div>
                          ):(
                            <div style={{padding:"6px 0 2px",fontSize:11,color:"var(--green)",fontWeight:600}}>✓ +1 chosen: {halfElfChosen.join(", ")}</div>
                          );
 }
 return <RaceEntry title={char.species} entry={libRace} traits={spc.traits} titleHidden cantripExtra={raceCardExtra}/>;
                          })())}
                        </div>
