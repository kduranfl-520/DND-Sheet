...
function App() {
 ...
 const raceCardExtra = isHuman?(
                      <div style={{padding:"8px 0 4px"}}>
                        <div style={{fontSize:12,fontWeight:600,color:"var(--text)",marginBottom:6}}>
                          Extra Language <span style={{fontSize:10,color:"var(--muted)",fontWeight:400}}>(Human trait)</span>
                        </div>
                        <select value={humanExtraLang}
 onChange={e=>{const langs=char.languages||[];setF("languages",[e.target.value,...langs.slice(1)]);}}
 ...
 return(<button key={stat} onClick={()=>{if(full)return;setF("halfElfBonusStats",chosen?halfElfChosen.filter(s=>s!==stat):[...halfElfChosen,stat]);}} style={{padding:"4px 12px",borderRadius:8,fontSize:12,fontWeight:700,border:"1px solid "+(chosen?"rgba(99,102,241,0.5)":full?"var(--divider)":"var(--border)"),background:chosen?"rgba(99,102,241,0.12)":full?"transparent":"var(--input)",color:chosen?"var(--accent)":full?"var(--subtle)":"var(--text)",cursor:full?"default":"pointer",fontFamily:"inherit"}}>{stat}{chosen&&" +1"}</button>);
                          })}
                        </div>
                        {halfElfChosen.length<2&&<div style={{fontSize:10,color:"var(--subtle)",marginTop:4,fontStyle:"italic"}}>{2-halfElfChosen.length} more to choose</div>}
                        {halfElfChosen.length===2&&<div style={{fontSize:11,color:"var(--green)",fontWeight:600,marginTop:4}}>✓ +1 chosen: {halfElfChosen.join(", ")}</div>}
                      </div>
                    ):null;
 return (
                      <>
 ...
 onClick={()=>setOpenTrait(openTrait==="race"?null:"race")}>
                          ...
                          {openTrait==="race"&&<RaceEntry title={char.species} entry={libRace} traits={spc.traits} titleHidden cantripExtra={raceCardExtra}/>}
                        </div>
