...
function App() {
 ...
 const Empty = ({icon,msg}) => <div className="empty-state"><div style={{fontSize:32,marginBottom:8}}>{icon}</div>{msg}</div>;
  ...
  // RENDER
 return (
    <div className="sheet" ref={el=>{if(el){const s=el.style;if(darkMode){s.setProperty("--bg","#0f1117");s.setProperty("--surface","#16191f");s.setProperty("--card","#1e2128");s.setProperty("--input","#1e2128");s.setProperty("--border","#2e3340");s.setProperty("--border-focus","#818cf8");s.setProperty("--text","#e2e8f0");s.setProperty("--muted","#94a3b8");s.setProperty("--subtle","#64748b");s.setProperty("--divider","#2e3340");}else{["--bg","--surface","--card","--input","--border","--border-focus","--text","--muted","--subtle","--divider"].forEach(v=>s.removeProperty(v));}}}} >
      <style>{CSS}</style>
 ...
 onClick={()=>setCharSaving(p=>({...p,languages:[p.languages[0],...p.languages.slice(1).filter((_,j)=>j!==i)]}))}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
          <button
            ...
            style={{width:30,height:18,borderRadius:9,border:"1px solid var(--border)",background:darkMode?"rgba(129,140,248,0.3)":"var(--input)",cursor:"pointer",display:"flex",alignItems:"center",padding:"0 2px",transition:"all .2s",position:"relative",flexShrink:0}}>
            <div style={{width:13,height:13,borderRadius:"50%",background:darkMode?"var(--accent)":"var(--muted)",transform:darkMode?"translateX(12px)":"translateX(0)",transition:"all .2s",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8}}>{darkMode?"🌙":"☀️"}</div>
          </button>
      ...
      <div className="dt-body">
        ...
        <div className="dt-colors">
          {[{id:"blue",hex:"#3b82f6"},{id:"purple",hex:"#a855f7"},{id:"red",hex:"#ef4444"},
            {id:"orange",hex:"#f97316"},{id:"green",hex:"#22c55e"},{id:"black",hex:"#374151"},{id:"gold",hex:"#eab308"},
