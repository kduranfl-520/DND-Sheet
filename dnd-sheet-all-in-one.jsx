...
function App() {
 ...
 return (
                            <span key={type+damage} className="ri-tag" style={racialStyle}>
                              {racial?<span className="ri-tag-type">RACE</span>:<span className="ri-tag-type">{type.slice(0,3).toUpperCase()}</span>}
                              {damage}
                              {!racial&&<button className="ri-tag-x" onClick={()=>removeEntry(type,damage)}>x</button>}
                            </span>
                          );
                        })}
                      </div>
 ...
 rollCheck("Breath Weapon",total);
                                            }}>{(()=>{const [num]=dmgDice.split("d");return(<span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:3}}>{num}<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"><path d="M2 5l6-3 6 3v6l-6 3-6-3z"/><path d="M2 5l6 3 6-3M8 8v6"/></svg>▶</span>);})()}</button>
                                        </div>
                                      </div>
                                    </div>
                                  );
