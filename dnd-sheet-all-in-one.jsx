...
function App() {
 ...
 const allSk=["Acrobatics","Animal Handling","Arcana","Athletics","Deception","History","Insight","Intimidation","Investigation","Medicine","Nature","Perception","Performance","Persuasion","Religion","Sleight of Hand","Stealth","Survival"];
 const hes=char.halfElfSkills||[];
 return(
                            <div style={{padding:"8px 14px 12px"}}>
                              <div style={{fontSize:11,fontWeight:600,color:"var(--text)",marginBottom:5}}>
                              ...
                              <div style={{display:"flex",flexDirection:"column",gap:5}}>
                                {[0,1].map(idx=>(
                                  <select key={idx} value={hes[idx]||""} onChange={e=>{const next=[...hes];next[idx]=e.target.value;setF("halfElfSkills",next.filter(Boolean));}}
 style={{width:"100%",background:"var(--input)",border:"1px solid "+(hes[idx]?"var(--green)":"var(--border)"),borderRadius:8,padding:"6px 10px",fontSize:12,color:hes[idx]?"var(--text)":"var(--subtle)",outline:"none",fontFamily:"inherit"}}>
                                    <option value="">-- Choose skill {idx+1} --</option>
                                    {allSk.filter(s=>s!==hes[idx===0?1:0]).map(s=>(<option key={s} value={s}>{s}</option>))}
                                  </select>
                                ))}
                              </div>
                            </div>
