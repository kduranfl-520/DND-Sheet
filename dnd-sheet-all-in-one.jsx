            ...
            <div className="card-wrap" style={{marginBottom:8,padding:0,overflow:"hidden"}}>
              ...
              {openWearing&&<div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:6}}>
                {[
                  ...
                  {label:"Body",key:"wearing_body",cats:["Armor","Magic Item"],filter:(e)=>e.category==="Armor"||e.title.toLowerCase().includes("robe")||e.title.toLowerCase().includes("armor")||e.title.toLowerCase().includes("vest")||e.title.toLowerCase().includes("tunic")||e.title.toLowerCase().includes("chain")||e.title.toLowerCase().includes("plate")||e.title.toLowerCase().includes("leather")||e.title.toLowerCase().includes("hide")||e.title.toLowerCase().includes("mail")},
                  {label:"Arms",key:"wearing_arms",cats:["Magic Item"],filter:(e)=>e.title.toLowerCase().includes("arm")||e.title.toLowerCase().includes("bracer")||e.title.toLowerCase().includes("sleeve")||e.title.toLowerCase().includes("gauntlet")||e.title.toLowerCase().includes("vambrace")},
                  {label:"Hands",key:"wearing_hands",cats:["Magic Item"],filter:(e)=>e.title.toLowerCase().includes("glove")||e.title.toLowerCase().includes("gauntlet")||e.title.toLowerCase().includes("hand")||e.title.toLowerCase().includes("ring")||e.title.toLowerCase().includes("mitten")},
                  ...
                  return (
                    <div key={key} style={{display:"flex",gap:4,alignItems:"center"}}>
                      ...
                      <select className="gi gi-sm" style={{flex:1,color:val?"var(--text)":"var(--subtle)"}}
                        value={val} onChange={e=>setF(key,e.target.value)}>
                        <option value="">— None —</option>
                        {opts.map(e=><option key={e.id} value={e.title}>{e.title}</option>)}
                      </select>
                      {val&&<button style={{fontSize:9,padding:"2px 6px",borderRadius:5,border:"1px solid var(--border)",background:"var(--input)",color:"var(--muted)",cursor:"pointer",flexShrink:0}}
                        onClick={()=>setF(key,"")}>✕</button>}
                    </div>
                  );
                })}
              </div>}
