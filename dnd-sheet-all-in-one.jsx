...
const BLANK = {
 ...
 hp:{max:0,current:1,temp:0},
  wearing_head:"",wearing_shoulders:"",wearing_body:"",wearing_arms:"",wearing_hands:"",wearing_hips:"",wearing_feet:"",
 ac:10, speed:30,
...
function App() {
 ...
 onClick={()=>setCharSaving(p=>{const a=[...(p.weaponItems||[])];a.splice(idx,1);return {...p,weaponItems:a};})}>✕</button>
            ...
            <div className="card-wrap" style={{padding:"10px 14px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[["Head","wearing_head"],["Shoulders","wearing_shoulders"],["Body","wearing_body"],["Arms","wearing_arms"],["Hands","wearing_hands"],["Hips","wearing_hips"],["Feet","wearing_feet"]].map(([label,key])=>(
                  <div key={key} style={{display:"flex",flexDirection:"column",gap:3}}>
                    <label style={{fontSize:9,fontWeight:700,letterSpacing:".07em",textTransform:"uppercase",color:"var(--muted)"}}>{label}</label>
                    {key==="wearing_head" ? (
                      <select className="gi" style={{fontSize:12,padding:"4px 8px"}}
                        value={char[key]||""}
                        onChange={e=>setF(key,e.target.value)}>
                        <option value="">— None —</option>
                        {libEntries.filter(e=>e.category==="Magic Item"&&(e.title.toLowerCase().includes("helm")||e.title.toLowerCase().includes("hat")||e.title.toLowerCase().includes("head")||e.title.toLowerCase().includes("crown")||e.title.toLowerCase().includes("circlet")||e.title.toLowerCase().includes("hood"))).map(e=>(
                          <option key={e.id} value={e.title}>{e.title}</option>
                        ))}
                      </select>
                    ) : (
                      <input className="gi" style={{fontSize:12,padding:"4px 8px"}}
                        value={char[key]||""}
                        onChange={e=>setF(key,e.target.value)}
                        placeholder={label}/>
                    )}
