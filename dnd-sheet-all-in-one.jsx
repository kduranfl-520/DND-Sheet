...
const BLANK = {
 ...
 photos:[],
  // Rounds tab
 conditions:[], concentrating:"", actionUsed:false, bonusActionUsed:false, reactionUsed:false, moveUsed:0, rageActive:false, rageRounds:0,
...
function App() {
        ...
        {tab==="rounds" && (
          <div className="panel">
            ...
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              {[
                {key:"actionUsed",     label:"Action",       color:"var(--accent)",  bg:"rgba(99,102,241,0.1)",  border:"rgba(99,102,241,0.35)"},
                {key:"bonusActionUsed",label:"Bonus Action", color:"var(--amber)",   bg:"rgba(245,158,11,0.1)",  border:"rgba(245,158,11,0.35)"},
                {key:"reactionUsed",   label:"Reaction",     color:"var(--green)",   bg:"rgba(34,197,94,0.1)",   border:"rgba(34,197,94,0.35)"},
                ...
                return (
                  <button key={key}
                    ...
                    <div style={{
                      width:20, height:20, borderRadius:"50%", border:"2px solid "+(used?"var(--border)":color),
                      background:used?"transparent":color, transition:"all .18s",
                    }}/>
                    <div style={{fontSize:13, fontWeight:800, color:used?"var(--muted)":color, letterSpacing:".01em"}}>{label}</div>
                    <div style={{fontSize:10, color:"var(--subtle)", fontWeight:500}}>{used?"Used":"Available"}</div>
                  </button>
                );
              })}
            </div>
 ...
 const increments = [5,10,15,20,25,30,35,40,45,50,55,60,65,70];
 ...
 border:"1px solid "+(isOver?"var(--divider)":isUsed?"rgba(99,102,241,0.4)":"var(--border)"),
 background:isOver?"transparent":isUsed?"rgba(99,102,241,0.12)":"var(--input)",
 color:isOver?"var(--border)":isUsed?"var(--accent)":"var(--muted)",
 cursor:isOver?"default":"pointer",fontFamily:"'DM Mono',monospace",
 transition:"all .15s",
 }}>{inc}</button>
                      );
                    })}
