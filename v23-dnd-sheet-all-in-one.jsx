...
function WearingCard({label,icon,slotKey,cats,filterFn,hand,char,setCharSaving,setF,libEntries,wearingInfo,setWearingInfo}) {
  const [open, setOpen] = (window.useState||React.useState)(false);
  const items = Array.isArray(char[slotKey]) ? char[slotKey] : [];
  const opts = libEntries.filter(e=>cats.includes(e.category)&&filterFn(e)).sort((a,b)=>a.title.localeCompare(b.title));
  ...
  return (
    <div className="card-wrap" style={{marginBottom:8,padding:0,overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 10px",background:"var(--surface)",borderBottom:open?"1px solid var(--divider)":"none",borderRadius:open?"8px 8px 0 0":"8px"}}>
        <div style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer",flex:1}} onClick={()=>setOpen(o=>!o)}>
          <span style={{fontSize:12}}>{icon}</span>
          <span style={{fontSize:10,fontWeight:700,letterSpacing:".07em",textTransform:"uppercase",color:"var(--muted)"}}>{label}</span>
          {items.length>0&&<span style={{fontSize:9,color:"var(--accent)",background:"rgba(99,102,241,0.1)",borderRadius:10,padding:"0 5px"}}>{items.length}</span>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <button style={{fontSize:11,fontWeight:700,color:"var(--accent)",background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.25)",borderRadius:6,padding:"1px 8px",cursor:"pointer"}}
            onClick={e=>{e.stopPropagation();setF(slotKey,[...items,{name:"",eq:false,at:false,hand:""}]);setOpen(true);}}>+ Add</button>
          <span style={{fontSize:10,color:"var(--muted)",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>{open?"▲":"▼"}</span>
        </div>
      </div>
      {open&&<div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:5}}>
        {items.length===0&&<div style={{fontSize:11,color:"var(--subtle)",fontStyle:"italic"}}>Nothing worn. Tap + Add.</div>}
        {items.map((item,idx)=>{
          ...
          const update = (patch)=>setCharSaving(p=>{
            const arr=[...(Array.isArray(p[slotKey])?p[slotKey]:[])];
            arr[idx]={name:itemName,eq:itemEq,at:itemAt,hand:itemHand,...patch};
            return {...p,[slotKey]:arr};
          });
          ...
          return (
            <div key={idx} style={{display:"flex",gap:4,alignItems:"center"}}>
              <button title={itemEq?"Equipped":"Not equipped"}
                ...
                onClick={()=>update({eq:!itemEq})}>EQ</button>
              {needsAttune&&<button title={itemAt?"Attuned":"Not attuned"}
                ...
                onClick={()=>update({at:!itemAt})}>AT</button>}
              {hand&&<button title={itemHand||"No hand"}
                ...
                onClick={()=>update({hand:hands[(hands.indexOf(itemHand)+1)%hands.length]})}>{itemHand||"—"}</button>}
              <select className="gi gi-sm" style={{flex:1,color:itemName?"var(--text)":"var(--subtle)"}}
                value={itemName||""} onChange={e=>update({name:e.target.value})}>
                <option value="">— None —</option>
                {opts.map(e=><option key={e.id} value={e.title}>{e.title}</option>)}
              </select>
              {itemName&&entry&&(
                <div style={{position:"relative",flexShrink:0}}>
                  <span className="ft-info-icon" style={{cursor:"pointer",fontSize:10,padding:"1px 5px",borderRadius:4,border:"1px solid rgba(99,102,241,0.3)",background:"rgba(99,102,241,0.08)",color:"var(--accent)"}}
              ...
              <button style={{fontSize:9,padding:"2px 6px",borderRadius:5,border:"1px solid var(--border)",background:"var(--input)",color:"var(--muted)",cursor:"pointer",flexShrink:0}}
                onClick={()=>setCharSaving(p=>{const arr=[...(Array.isArray(p[slotKey])?p[slotKey]:[])];arr.splice(idx,1);return {...p,[slotKey]:arr};})}>✕</button>
            </div>
          );
        })}
      </div>}
...
function App() {
 ...
 onClick={()=>setCharSaving(p=>{const a=[...(p.weaponItems||[])];a.splice(idx,1);return {...p,weaponItems:a};})}>✕</button>
            ...
            {/* Head */}
            <WearingCard
              label="Head" icon="👘" slotKey="wearing_head"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("helm")||e.title.toLowerCase().includes("hat")||e.title.toLowerCase().includes("head")||e.title.toLowerCase().includes("crown")||e.title.toLowerCase().includes("circlet")||e.title.toLowerCase().includes("hood")}
              hand={false}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Neck */}
            <WearingCard
              label="Neck" icon="👘" slotKey="wearing_neck"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("necklace")||e.title.toLowerCase().includes("amulet")||e.title.toLowerCase().includes("neck")||e.title.toLowerCase().includes("pendant")||e.title.toLowerCase().includes("medallion")||e.title.toLowerCase().includes("periapt")||e.title.toLowerCase().includes("scarab")}
              hand={false}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Shoulders */}
            <WearingCard
              label="Shoulders" icon="👘" slotKey="wearing_shoulders"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("shoulder")||e.title.toLowerCase().includes("mantle")||e.title.toLowerCase().includes("pauldron")||e.title.toLowerCase().includes("cloak")||e.title.toLowerCase().includes("cape")}
              hand={false}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Body */}
            <WearingCard
              label="Body" icon="👘" slotKey="wearing_body"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("robe")}
              hand={false}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Arms */}
            <WearingCard
              label="Arms" icon="👘" slotKey="wearing_arms"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("arm")||e.title.toLowerCase().includes("bracer")||e.title.toLowerCase().includes("sleeve")||e.title.toLowerCase().includes("gauntlet")||e.title.toLowerCase().includes("vambrace")}
              hand={true}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Hands */}
            <WearingCard
              label="Hands" icon="👘" slotKey="wearing_hands"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("glove")||e.title.toLowerCase().includes("gauntlet")||e.title.toLowerCase().includes("hand")||e.title.toLowerCase().includes("ring")||e.title.toLowerCase().includes("mitten")}
              hand={true}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Hips */}
            <WearingCard
              label="Hips" icon="👘" slotKey="wearing_hips"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("belt")||e.title.toLowerCase().includes("girdle")||e.title.toLowerCase().includes("hip")}
              hand={false}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
            {/* Feet */}
            <WearingCard
              label="Feet" icon="👘" slotKey="wearing_feet"
              cats=["Magic Item"]
              filterFn={(e)=>e.title.toLowerCase().includes("boot")||e.title.toLowerCase().includes("slipper")||e.title.toLowerCase().includes("shoe")||e.title.toLowerCase().includes("feet")||e.title.toLowerCase().includes("sandal")}
              hand={false}
              char={char} setCharSaving={setCharSaving} setF={setF}
              libEntries={libEntries} wearingInfo={wearingInfo} setWearingInfo={setWearingInfo}
            />
