...
function App() {
 ...
 style={isClass?{background:"rgba(99,102,241,0.08)",borderRadius:5,outline:"1px solid rgba(99,102,241,0.2)"}:isBg?{background:"rgba(59,130,246,0.08)",borderRadius:5,outline:"1px solid rgba(59,130,246,0.2)"}:isVHuman?{background:"rgba(245,158,11,0.08)",borderRadius:5,outline:"1px solid rgba(245,158,11,0.25)"}:{}}
 ...
 const cantripExtra = (subraceCantrip||subraceLangCount>0||toolChoiceOptions||isHalfElf||isHuman) ? (
                                  <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                                    {isHalfElf&&(
                                      <div style={{flex:"1 1 100%"}}>
                                        <div style={{fontSize:12,fontWeight:600,color:"var(--text)",marginBottom:6}}>
                                          Ability Score Increase <span style={{fontSize:10,color:"var(--muted)",fontWeight:400}}>(+1 to two stats, not CHA)</span>
                                        </div>
 ...
 onClick={()=>{
 if(full) return;
 const next = chosen?halfElfChosen.filter(s=>s!==stat):[...halfElfChosen,stat];
 setF("halfElfBonusStats",next);
                                                }}
 style={{
 padding:"4px 12px",borderRadius:8,fontSize:12,fontWeight:700,
 border:"1px solid "+(chosen?"rgba(99,102,241,0.5)":full?"var(--divider)":"var(--border)"),
 background:chosen?"rgba(99,102,241,0.12)":full?"transparent":"var(--input)",
 color:chosen?"var(--accent)":full?"var(--subtle)":"var(--text)",
 cursor:full?"default":"pointer",fontFamily:"inherit",transition:"all .15s",
 }}>
                                                {stat} {chosen&&"+1"}
                                              </button>
                                            );
                                        ...
                                        {halfElfChosen.length<2&&(
                                          <div style={{fontSize:10,color:"var(--subtle)",marginTop:4,fontStyle:"italic"}}>
                                            {2-halfElfChosen.length} more to choose
                                          </div>
                                        )}
                                      </div>
                                    )}
                                    {isHuman&&(
                                      <div style={{flex:"1 1 100%"}}>
                                        <div style={{fontSize:12,fontWeight:600,color:"var(--text)",marginBottom:6}}>
                                          Extra Language <span style={{fontSize:10,color:"var(--muted)",fontWeight:400}}>(choose one)</span>
                                        </div>
                                        <select
 value={humanExtraLang}
 onChange={e=>{
 const langs = char.languages||[];
 const rest = langs.slice(1);
 setF("languages",[e.target.value,...rest]);
                                          }}
 style={{width:"100%",background:"var(--input)",border:"1px solid var(--border)",borderRadius:8,padding:"6px 10px",fontSize:12,color:humanExtraLang?"var(--text)":"var(--subtle)",outline:"none",fontFamily:"inherit",borderColor:humanExtraLang?"var(--green)":"var(--border)"}}>
                                          <option value="">-- Choose a language --</option>
                                          {ALL_LANGUAGES.filter(l=>l!=="Common").map(l=>(
 ...
 style={{borderColor:"rgba(99,102,241,0.4)",color:(char.languages||[])[0]?"var(--accent)":"var(--muted)",width:"100%"}}>
                                          <option value="">-- Choose a language --</option>
                                          {ALL_LANGUAGES.filter(l=>!autoLangs.includes(l)).map(l=>(
                                            <option key={l} value={l}>{l}</option>
                                          ))}
