                    ...
                    <div key={key} style={{display:"flex",flexDirection:"column",gap:3}}>
                      <div style={{display:"flex",alignItems:"center",gap:4}}>
                        ...
                        <button style={{fontSize:9,fontWeight:700,padding:"1px 7px",borderRadius:5,border:"1px solid rgba(99,102,241,0.3)",background:"rgba(99,102,241,0.08)",color:"var(--accent)",cursor:"pointer",flexShrink:0}}
                          onClick={()=>setF(key,[...(Array.isArray(char[key])?char[key]:[]),{name:"",eq:false,at:false,hand:""}])}>+ Add</button>
                      </div>
                      {(Array.isArray(char[key])?char[key]:[]).map((item,itemIdx)=>{
                        ...
                        return (
                          <div key={itemIdx} style={{display:"flex",gap:4,alignItems:"center"}}>
                            ...
                            {itemName&&(()=>{
                              const infoKey = key+"-"+itemIdx;
                              const entry = libEntries.find(ex=>ex.title===itemName&&cats.includes(ex.category));
                              if(!entry) return null;
                              const infoText = [entry.desc,entry.notes].filter(Boolean).join(" | ");
                              return (
                                <div style={{position:"relative",flexShrink:0}}>
